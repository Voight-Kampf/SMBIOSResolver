...
//		if (!strncmp(cpuid_vendor, CPUID_VID_INTEL, sizeof(CPUID_VID_INTEL)) && (cpuid_info()->cpuid_features & CPUID_FEATURE_SSE2)) fsbFrequency /= 4;
//	}
//
	...
	if(dictString) 
	{
		...
		{
			if (!strncmp(cpuid_vendor, CPUID_VID_INTEL, sizeof(CPUID_VID_INTEL)))
			{
				if ((cpuid_family == 0x0f) && (cpuid_model >= 2))
				{
					msr = rdmsr64(0x0000002C);
					switch ((msr >> 16) & 0x7) {
						case 0:
							if (cpuid_model == 2) fsbFrequency = 100 * 1000000;
							else 
							{
...
UInt64 SMBIOSResolver::calcCPU(UInt64 fsb_frequency_hz)
{
	...
	if (!strncmp(cpuid_vendor, CPUID_VID_INTEL, sizeof(CPUID_VID_INTEL)))
	{
		if(((cpuid_family == 0x06) && ((cpuid_model > 0x0d)) ||
			((cpuid_family == 0x0f) && (cpuid_model > 0x02))))
		{
	...
	else if((!strncmp(cpuid_vendor, CPUID_VID_AMD, sizeof(CPUID_VID_AMD))) && (cpuid_family == 0x0f))
	{
		if(cpuid_extfamily == 0x00 /* K8 */)
			coef = (rdmsr64(K8_FIDVID_STATUS) & 0x3f) / 2 + 4;
		else if(cpuid_extfamily == 0x01 /* K10 */)
		{
